...
class CatConv2d(nn.Module):
    def __init__(self, in_channels1, in_channels2, out_channels, kernel_size=3, stride=1, padding=1):
        super(CatConv2d, self).__init__()
        
        # The concatenation convolution, where we concatenate two inputs and apply a 2D convolution
        self.conv = nn.Conv2d(in_channels1 + in_channels2, out_channels, kernel_size, stride, padding)
        
    def forward(self, x1, x2):
        # Concatenate along the channel dimension (dim=1 for channels)
...
class HarDNet(nn.Module):
    def __init__(self, nclasses=20, input_dim=5, aux=False):
...
#         self.shortcut_layers = [3, 4, 7, 10]
        ...
        self.base.append(ConvLayer(in_channels=input_dim, out_channels=first_ch[0], kernel=3))
...
if __name__ == "__main__":
    import time
    model = HarDNet(20, input_dim=7, aux=False).cuda()
    ...
    with torch.no_grad():
        for i in range(100):
            inputs = torch.randn(1, 7, 128, 2048).cuda()
            start.record()
