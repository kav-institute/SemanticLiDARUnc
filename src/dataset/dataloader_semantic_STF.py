...
def main():
    import glob
    import cv2 
    data_path_test = [(bin_path, bin_path.replace("velodyne", "labels").replace("bin", "label")) for bin_path in sorted(glob.glob(f"/home/devuser/workspace/data/SemanticSTF/train/velodyne/*.bin"))]

    depth_dataset_test = SemanticSTF(data_path_test, rotate=False, flip=False, projection=(64,512),resize=False)
