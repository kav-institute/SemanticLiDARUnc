...
class EquirectangularRayCasting:
    def __init__(self, height=512, origin=(0,0,0)):
...
if __name__ == "__main__":
    ...
    mesh = o3d.io.read_triangle_mesh(os.path.join(save_path,"kitti_vdbmesh_raw.ply"))

    ray_caster = EquirectangularRayCasting()
    ray_caster.createScene()
    ray_caster.addMesh(mesh)
    for scan, key, labels, pose in tqdm(dataset):
        T = np.eye(4)
        T = pose
        rc, rays = ray_caster.rayCast(T=pose)
        ...
        color_img = ray_caster.get_colors(mesh, rc['primitive_ids'].numpy())
        prev_sem_pred = cv2.applyColorMap(np.uint8(np.round(20*color_img[...,1])), custom_colormap)
        cv2.imshow("prev_sem_pred", prev_sem_pred[...,::-1])
        cv2.imshow("reflectivity", cv2.applyColorMap(np.uint8(255*color_img[...,0]),cv2.COLORMAP_JET))
        cv2.waitKey(0)
